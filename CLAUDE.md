...
- `MarkdownToolbar` — 텍스트 선택 후 버튼으로 서식 적용(좌측 끝 CLR은 서식 제거). 카테고리는 드롭다운(기존 목록 + "새 카테고리" prompt). **GRAPH 버튼**(IMG 옆) → `GraphComposer` 창: |작성 가능 그래프|적용 가능 속성|속성 값| 3열(각 열 스크롤)에서 kind 선택 → 속성 적용(필수 자동, 의존 속성 동반 적용) → 값 기입(미기입 시 자기 일관 기본값) → 커서 위치에 ```graph boilerplate 삽입. 미선택 적용 시 모달 알림. 새 kind·속성 추가 시 `GraphComposer.tsx`의 카탈로그도 갱신할 것.
