...
- 함수 그래프 개선 (2026-07-09 5차): 다중 서브플롯 — `[[plot]]` 1~4개가 작성
  순서대로 배치(2개: 2×1, 3~4개: 2×2, 3개면 넷째 칸 빈 칸, 768px 이하 1열).
  구역마다 개별 title(다크 타이틀 바)·fn·domain·range·integral과 호버 readout을
  갖고, 최상위 domain/range는 공통 기본값으로 상속되며, 모든 구역이 `[params]`
  슬라이더 한 세트에 동기화된다. 구현: SubPlot 컴포넌트 분리(구역별 ResizeObserver
  실측 — 배치는 CSS grid 전담), 단일 fn 스펙은 plots[0]으로 정규화되어 기존 게시물
  무영향. 테스트: 단위원 각도 t에 따른 sin·cos 2×1 그래프(∫ 음영 동기화).
- 함수 그래프 개선 (2026-07-09 6차): plot 종류 `kind = "circle"` 추가 — 단위원 +
  param 각도 식(`angle`, 라디안)을 따라 회전하는 반지름(alert 색) + 끝점·축 사영
  점선 + θ°/cos/sin readout. 가로 정의역을 종횡비로 보정해 원형 유지. 테스트
  게시물의 단위원 섹션을 도(°) 단위로 개편: 1번 칸 단위원, 2·3번 칸 sin(x°)·cos(x°)
  (domain [0, 360], param t 0~360°), 3플롯 2×2 배치(넷째 칸 빈 칸).
- 함수 그래프 개선 (2026-07-09 7차): readout을 항목당 한 줄로 변경(좁은 셀에서
  한 줄 병기가 잘리던 문제 해결) + `display.항목 = true/false` 키로 항목별 표시
  선택(기본 전부 true — fn plot: x·fx·integral, circle plot: theta·cos·sin.
  다중 모드에서는 각 plot 안에 지정). 호버 안내 문구는 제거.
- 함수 그래프 개선 (2026-07-09 8차): display 항목을 모든 그래프 종류의 시각 요소로
  일반화 — readout 줄과 대응 요소를 쌍으로 제어. fn: x=호버 크로스헤어,
  fx=호버 점, integral=음영·경계선(false면 계산째 생략) / circle: theta=반지름·끝점,
  cos=x축 사영 점선, sin=y축 사영 점선. 스펙 키 변화 없음(하위 호환).
- 함수 그래프 개선 (2026-07-09 9차): ① display 기본값을 false로 전환(명시한 것만
  표시 — 미배포 시점의 명시적 의미 변경, 게시물 스펙 동시 갱신) ② `display.integral`
  (∫ 값 readout)과 `display.graph.integral`(음영·경계선 시각화) 분리 ③ param 추적점 —
  plot 키 `point = "<param 식>"` + `display.graph.point`: 곡선 위 링 마커가 슬라이더를
  따라 이동하고 비호버 시 x·fx readout이 추적점 값을 표시(호버 우선). 단위원 섹션의
  sin·cos 칸에 point = "t" 추적 데모 적용.
- 함수 그래프 리팩토링 (2026-07-09 10차, 동작 무변경): graphSpec.ts →
  `src/lib/graph/`(index 오케스트레이션 + kinds/fn·circle 모듈 + contract 계약 +
  features/integral·point + primitives·params·sampling), FunctionGraph.tsx →
  `src/components/post/graph/`(컨테이너 + SubPlot 디스패치 + kind별 SubPlot +
  ParamControls + 공용 훅·상수). kind 레지스트리는 리터럴 객체 + 판별 유니언
  병존(소진성 검사 보존), 새 kind 추가는 파일 신설 + 등록 3곳. 문법 계약(devnotes
  §2.5)은 이 구조의 상위 공개 API로 유지.
- 에디터 그래프 삽입 창 (2026-07-09 11차): 도구바 IMG 옆 GRAPH 버튼 →
  `GraphComposer` — |작성 가능 그래프|적용 가능 속성|속성 값| 3열(각 열 스크롤).
  kind 선택 시 해당 kind의 전체 속성 노출, 속성 클릭 시 적용 표시(▣) + 값 양식
  (필수 속성은 자동 적용·해제 불가, display.graph.point→point 등 의존 속성 동반
  적용). 적용 버튼: 미선택이면 모달 알림, 속성/값 미기입이면 자기 일관 기본값
  (어떤 조합도 파싱 가능)으로 ```graph boilerplate를 커서 위치에 삽입.
- 그래프 엔진 전면 개정 (2026-07-10 12차): 목표 — 중·고·대학 수학 교재의
  시각화 대부분을 표현·상호작용. 두 축의 재건축:
  ① **표기: TOML 폐기 → 수학 문장 DSL** (smol-toml 제거 — 외부 파서 의존성 0).
  줄 하나 = 문장 하나, 수식이 곧 스펙(`f(x) = x^3 - 3x`). 분류는 좌변 형태 +
  자유변수의 결정적 규칙, 판별 불능은 행 번호를 짚는 시끄러운 오류. 라벨
  (`C: x^2 + y^2 = r^2`), param(`t = 1 : [0, 10]`), 지시문(view·---·animate·
  hide·show), 도구 문장(함수형)으로 구성 — 명세 전문은 devnotes.
  ② **아키텍처: kind 모델 폐기 → 좌표계 + 아이템 조합(Scene IR)**. 구역(---)
  하나에 곡선·영역·보조선·점·readout이 자유 조합된다(구 kind = "circle"은
  음함수 원 + vector + segment 조합으로 재현 — 프리셋으로 강등).
  구현 계층: `src/lib/expr/`(식 AST·컴파일러 + **기호 미분**(수치 폴백)·if·sum·
  조합함수·곱셈 생략·유니코드 별칭) → `src/lib/scene/`(2-pass 문장 파서 →
  Scene IR — 문법·렌더 간 방화벽) → `src/lib/plot/`(적응형 샘플링 이식 +
  **마칭 스퀘어**(음함수·부등식 영역, 자체 구현) + 심프슨 + 이분법 근 찾기) →
  `src/components/post/graph/`(SectionPlot 아이템 디스패치 렌더 + animate rAF).
  신규 능력: 다중 곡선(구분색 4종), 음함수·부등식 영역, 극·매개변수 곡선, 수열,
  방향장, 도함수 곡선(f'), 접선/할선(기호 미분), 리만합, 곡선 사이 넓이, 교점
  자동 마커, 자유·곡선 추적점, 벡터/선분/직선/텍스트, param 애니메이션(기본
  정지·▶ 토글), hide/show 표시 제어(기본: 선언한 것의 자연 표시 on).
  GraphComposer는 문장 팔레트 + 프리셋 + 편집 가능 미리보기로 재작성, 테스트
  게시물도 DSL로 재작성(11개 펜스 — 신규 기능 데모 포함). 검증: 파서 스모크
  테스트 33건 + Playwright 런타임 검증(교점 좌표·리만합·∫값 수치 대조,
  콘솔 오류 0, 기존 게시물 회귀 통과). FunctionGraph 청크 gzip ~25kB.
