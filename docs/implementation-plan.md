...
로컬 개발 서버 전용 (정적 호스팅에서는 파일 쓰기가 불가능하므로). 프로덕션 빌드에서는
안내 페이지만 남고 에디터 로직은 번들에서 제거된다.
...
- 팔레트는 `nier.css`의 CSS 변수(`--nier-*`)로 정의: 배경 `#cdc8b0`, 패널 `#dcd8c2`,
  다크 `#4c4a43`, 텍스트 `#454138`, 경고 `#b05a4a` 등.
- 공용 `Panel`(다크 타이틀바 + 밝은 본문), 그리드 배경 + 하단 파형 장식, 텍스트-배경 반전 호버.
